...
Course: CSCI-4500
Description: This program creates eight separate POSIX threads and calculates pi using
...
Notes - I had an issue with my counter variable and passing parameters through the threads to keep track of their number.
If I moved my counter variable outside of the mutex, it would be accessed at unrestricted times, and if i passed a parameter
from main it would resolve to integer division. Over 2 years since writing C takes some getting used to again! Only missing
lock/unlock every ten permutations.
*/ 
